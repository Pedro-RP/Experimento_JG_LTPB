addpath(genpath('C:\Users\numec\Desktop\Git-20220921T130535Z-001\Git'))
...
for k = 1:length(ecolumnmarker)
    ...
    if ~isempty(strfind(rowcontent,'.csv')) %contains(rowcontent,'.csv')
        aux = aux+1;
        b = strfind(rowcontent,'</td>');
        fileline = rowcontent( b(1):b(2) );
        hyperfile = fileline(10: end-1); hyperaux = strfind(hyperfile, '"');
        auxstr = strcat (url,hyperfile( hyperaux(1)+1: hyperaux(2)-1 ));
        datatables{aux,1} = webread(auxstr);
