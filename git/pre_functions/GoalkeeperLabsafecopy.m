function GoalkeeperLab(data,pathtogit,ntrials)

set(0,'defaultfigurecolor',[1 1 1]); set(0, 'DefaultFigureRenderer', 'painters');
addpath(genpath(pathtogit))

strfig.data = data; % stores the data in the figure
assignin('base','aux',0); % assigns the variable aux with the value 0
...
%%%%% Changed for MATLAB 2015            
%            if abs(grafiks(a).Position(1)-0.7) < 10^(-10)
%              delete(grafiks(a))
%            end
%%%%%
             if (length(grafiks(a).Tag) ==5 )&&(strcmp(grafiks(a).Tag(1:5),'axes3'))
               delete(grafiks(a))
             end
        end
    end 
    %ax3 = axes(f,'Position', [0.7 0.075 0.25 0.30]); ax3.Tag = 'axes3';
    ax3 = axes('Position', [0.7 0.075 0.25 0.30]); ax3.Tag = 'axes3'; % SUBSTITUTES THE LINE ABOVE IN MATLAB2015
    sses = findobj('Tag','axes3');
    boxplot(ax3,distws,groupsws)
    ylabel('time(sec.)')
    end
...
%         delete(pnorthw); delete(psouthw);
    ...
    ax1 = axes('Position', [ 0.05 0.55 0.375 0.4]); ax1.Tag = 'axes1'; % SUBSTITUTES THE LINE ABOVE IN MATLAB2015
...
%    ax2 = axes(f,'Position', [0.05 0.075 0.25 0.40]); ax2.Tag = 'axes2';
    ax2 = axes('Position', [0.05 0.075 0.25 0.40]); ax2.Tag = 'axes2'; % SUBSTITUTE THE LINE ABOVE IN MATLAB2015
