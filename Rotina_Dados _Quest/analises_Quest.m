...
for par = 1:(size(ZC,1)/1000);
    ...
    MAC1(par) = AC1;  %MAC1 é um vetor em que cada termo corresponde a média de acurácia de um participante no primeiro bloco.
    AC1=0;
...
for par = 1:(size(ZL,1)/1000);
    ...
    MAC4(par) = AC4;  %MAC1 é um vetor em que cada termo corresponde a média de tempos de um participante no primeiro bloco.
    AC4=0;
...
% % 
% TME=[MAC1,MAC3];%comparação grupo controle bloco 1&3
% C1xC3=kruskalwallis(TME.');%5
% 
% TMF=[MAC2,MAC3];%comparação grupo controle bloco 2%3
% C2xC3=kruskalwallis(TMF.');%6

% TMG=[MAC4,MAC5];%comparação grupo ltpb bloco 1&2
% L1xL2=kruskalwallis(TMG.');%7
%  
% TMH=[MAC4,MAC6];%comparação grupo ltpb bloco 1&3
% L1xL3=kruskalwallis(TMH.');%8
% % 
% TMI=[MAC5,MAC6];%comparação grupo ltpb bloco 2&3
