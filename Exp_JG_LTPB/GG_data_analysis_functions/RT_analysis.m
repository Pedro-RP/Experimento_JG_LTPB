...
RTCL = 0; %Response Time Control in the Learning Phase
for par = 1:(size(data_control,1)/1000)
...
rt.response_times.LTPB.learning_phase = MRTLL;
rt.response_times.LTPB.prediction_phase = MRTLP;


%%% Comparation between groups

%Boxplot
MRT1=[MRTCL MRTLL]; %Mean response times block 1
MRT2=[MRTCP MRTLP];

MRTF = [MRT1 MRT2]; %Mean Response Times Full -> arrange the data in a single vector so the boxplot function can work.

control_n = (size(data_control,1)/1000);
LTPB_n = (size(data_LTPB,1)/1000); %number of participants in each group

grp =[zeros(1,control_n),ones(1,LTPB_n),2*ones(1,control_n),3*ones(1,LTPB_n)]; %grouping variable. 
...
title('Distribution of the Mean Response Times of each group in each phase');
ylabel("Mean Response Time(s)");
ylim([0 2.5])
yticks([0:0.2:2.5])
xticks([1 2 3 4 5 6])
xticklabels({'Learning Phase - Control','Learning Phase - LTPB', 'Prediction Phase - Control', 'Prediction Phase - LTPB'})
xline(2.5)
...
[~,~,pCL] = swtest_norm(MRTCL.'); %Shapiro-Wilk test.
[~,~,pCP] = swtest_norm(MRTCP.');
[~,~,pLL] = swtest_norm(MRTLL.'); 
[~,~,pLP] = swtest_norm(MRTLP.');


rt.between_comparasion.assumption_check.norm_check.pCL = pCL;
rt.between_comparasion.assumption_check.norm_check.pCP = pCP;
rt.between_comparasion.assumption_check.norm_check.pLL = pLL;
rt.between_comparasion.assumption_check.norm_check.pLP = pLP;

% Check if the variances are equal between each distribuction

[~, pFL] = vartest2 (MRTCL, MRTLL);
[~, pFP] = vartest2 (MRTCP, MRTLP);

rt.between_comparasion.assumption_check.var_check.pFL = pFL;
rt.between_comparasion.assumption_check.var_check.pFP = pFP;
...
[pL, ~] = ranksum (MRTCL, MRTLL); % doing the 2-sample t-test for each block.
[pP, ~] = ranksum (MRTCP, MRTLP);


rt.between_comparasion.w_test.pL = pL;
rt.between_comparasion.w_test.pP = pP;
...
eL = (MMRTLL * 100)/MMRTCL;
eP = (MMRTLP * 100)/MMRTCP;


rt.between_comparasion.effect_size.relative.eL = eL;
rt.between_comparasion.effect_size.relative.eP = eP;

% Calculating Hedge's g effect size values

 mesL = mes(MRTLL.',MRTCL.','hedgesg');
 mesP = mes(MRTLP.',MRTCP.','hedgesg');

hgL = mesL.hedgesg;
hgP = mesP.hedgesg;

rt.between_comparasion.effect_size.hedges_g.hgL = hgL;
rt.between_comparasion.effect_size.hedges_g.hgP = hgP;
...
% % Checking normality of the data samples
% 
% [~,~,pL1] = swtest_norm(MRTLL.'); %Shapiro-Wilk test.
% [~,~,pL2] = swtest_norm(MRTLP.');
...
% % F-test to check if the population variances are equal.
% 
% [~, pLF1_2] = vartest2 (MRTLL, MRTLP);
% [~, pLF1_3] = vartest2 (MRTLL, MRTL3);
% [~, pLF2_3] = vartest2 (MRTLP, MRTL3);
% 
...
% 
% [~,pL12] = ttest(MRTLL, MRTLP);
% pL1_2 = pL12*3; %bonferoni correction for multiple comparasions
% 
% [~,pL13] = ttest(MRTLL, MRTL3);
% pL1_3 = pL13*3; 
% 
% [~,pL23] = ttest(MRTLP, MRTL3);
...
% MMRTL3 = mean (MRTL3);
% 
% eL1_2 = (MMRTLL * 100)/MMRTLP;
% eL1_3 = (MMRTLL * 100)/MMRTL3;
% eL2_3 = (MMRTLP * 100)/MMRTL3;
% 
...
% % Calculating Hedge's g effect size values
% 
% mesL1_2 = mes(MRTLL.',MRTLP.','hedgesg');
% mesL1_3 = mes(MRTLL.',MRTL3.','hedgesg');
% mesL2_3 = mes(MRTLP.',MRTL3.','hedgesg');
% 
